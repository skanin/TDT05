...
class Dataset:
    ...
    def __init__(self, train_path : str, test_path : str) -> None:
        ...
        self.numeric_with_outliers_transformer = make_pipeline(
        ...
        numerical_transformer_cols = self.numerical_columns + self.hex_columns + self.ordinal_columns
        categorical_transformer_cols = self.categorical_columns
        ...
        self.preprocessor = make_column_transformer(
            ...
            (self.ordinal_transformer, self.ordinal_cat_columns + self.binary_columns),
        )
    ...
    def balance(self, test=True):
        sm = SMOTE(sampling_strategy='minority', random_state=7)

        self.X_train, self.y = sm.fit_resample(self.X_train, self.y)


    def load_train(self, dataset : str, delimiter = ',') -> None:
        self.df_train = pd.read_csv(dataset, delimiter=delimiter)
        self.X_train = self.df_train.drop(columns=['id', 'target'])
        self.y = self.df_train['target']
        self.X = self.df_train.drop(columns=['id', 'target'])

        self.preprocess_train(self.X_train)

    def load_test(self, dataset : str, delimiter = ',') -> None:
        self.df_test = pd.read_csv(dataset, delimiter = delimiter)
        self.X_test = self.df_test.drop(columns=['id'])
    ...
    def conv_bool(self, df : pd.DataFrame):
        ...
        # for col in self.ordinal_cat_columns:
        #     if col not in list(df.columns):
        #         continue
        #     col_loc = list(df.columns).index(col)
    ...
    def remove_rows(self, df):
        df = df[~df['f27'].isin(['g', 'i', 'j'])]
        df = df[~df['f23'].isin(['B', 'C'])]
        df = df[~df['f17'].isin([0.2, 0.3])]
        df = df[~df['f16'].isin([10])]
        df = df[~df['f0'].isin(['W', 'Z', 'S', 'G', 'V', 'J', 'L'])]
        df = df[~df['f8'].isin([2.0, 3.0])]
...
if __name__ == '__main__':
    ...
    classifiers = {
        ...
        'lgbm': LGBMClassifier(),
        'rf': RandomForestClassifier(n_estimators=100),
        'catboost': CatBoostClassifier(iterations=10000, learning_rate=1, depth=1)
    }
    ...
    clf = CatBoostClassifier(depth = 1, iterations = 10000, learning_rate = 1)
    # clf = AdaBoostClassifier(n_estimators=1000)
    # clf = StackingClassifier(estimators = [('ada', classifiers['adaboost']), ('lgbm', classifiers['lgbm'])], final_estimator = classifiers['catboost'])
    ...
    print_accuracy(prediction, y_test_split, 'Catboost')
